...
- `src/content/articles/*.md` / `*.mdx` — 記事のメタデータ（タイトル、カテゴリ、出典など）と解説文
...
### グラフと解説を交互に配置したい場合（MDX）

グラフが1つだけの記事は上記の`.md` + ページ側で`slot="chart"`に流し込むパターンで十分だが、複数のグラフそれぞれに個別の解説を挟みたい記事は`.mdx`拡張子を使う。frontmatter直後に`import`でコンポーネント・データを読み込み、`export const`で計算した値を、本文中の好きな位置に`<LineChart ... />`のように直接埋め込める。この場合ページ側（`.astro`）はメタデータの受け渡しだけで済み、データ計算・グラフ配置はすべて`.mdx`ファイル側に書く（`gan-shibou-kensho.mdx`が実例）。
...
e-Stat以外の公的機関（例：国立がん研究センターが公開するExcelファイル）が出典の場合も同じパターンで、APIキー不要なら `fetch-cancer-incidence.js` のように直接ダウンロード＋パースするスクリプトにする。

新しいデータセットを追加するには `scripts/fetch-*.js` を参考にスクリプトを作り、`package.json` にnpm scriptを追加し、`.github/workflows/update-data.yml` にステップを追記する。
